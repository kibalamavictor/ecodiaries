...
function SolutionsAtlasFallback({
  projects,
  selected,
  onSelect,
}: {
  projects: AtlasProject[]
  selected: AtlasProject | null
  onSelect: (project: AtlasProject | null) => void
}) {
...
export function SolutionsAtlasHero({ projects }: SolutionsAtlasHeroProps) {
  ...
  const [MapView, setMapView] = useState<ComponentType<AtlasMapProps> | null>(null)
  const [failed, setFailed] = useState(false)
  const [ready, setReady] = useState(false)

  useEffect(() => {
    let cancelled = false
    import('@/components/solutions/atlas/AtlasMap')
      .then((mod) => {
        if (!cancelled) setMapView(() => mod.AtlasMap)
      })
      .catch(() => {
