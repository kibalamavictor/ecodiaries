...
import { MobilePodcastCard } from '@/components/mobile/MobilePodcastCard'
...
export function MobileHome({ data }: MobileHomeProps) {
  ...
  const episodeCollage = collageImages(
    latestEpisodes.slice(MOBILE_COLLAGE_START, MOBILE_COLLAGE_END).map((e) => e.thumbnail),
  )
  const videoCollage = collageImages(
    latestVideos.slice(MOBILE_COLLAGE_START, MOBILE_COLLAGE_END).map((v) => v.image),
  )
  ...
  return (
    <div className="mobile-home-tailwind bg-paper pb-24 md:hidden">
      ...
      <MobileHomeSection title="Listen — youth climate dispatch" href="/listen" eyebrow="Podcasts">
        {latestEpisodes.length > 0 ? (
          <MobileScrollRow
            seeMore={{
              label: 'Podcasts',
              countText: `All ${counts.episodesCount} episodes`,
              href: '/listen',
              images: episodeCollage,
            }}
          >
            {latestEpisodes.slice(0, MOBILE_VISIBLE_CARDS).map((episode) => (
              <MobilePodcastCard key={episode.slug} episode={episode} />
            ))}
          </MobileScrollRow>
        ) : null}
      </MobileHomeSection>

      <MobileHomeSection title="Watch — field documentaries" href="/watch" eyebrow="Video">
        {latestVideos.length > 0 ? (
          <MobileScrollRow
            seeMore={{
              label: 'Documentaries',
              countText: `All ${counts.videosCount} videos`,
              href: '/watch',
              images: videoCollage,
            }}
          >
            {latestVideos.slice(0, MOBILE_VISIBLE_CARDS).map((video) => (
              <MobileVideoCard key={video.slug} video={video} />
            ))}
          </MobileScrollRow>
        ) : null}
      </MobileHomeSection>
